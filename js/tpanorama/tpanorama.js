(function (global, undefined) {
  ...
  function onDocumentTouchStart(event) {
    ...
    if (event.touches.length === 1) {
      event.preventDefault();
      _isUserInteracting = true;
      _onPointerDownPointerX = event.touches[0].clientX;
      _onPointerDownPointerY = event.touches[0].clientY;
      _onPointerDownLon = _lon;
      _onPointerDownLat = _lat;
    }
    // 双指触摸，用于缩放
    else if (event.touches.length === 2) {
      _isUserInteracting = false; // 缩放时停止拖拽交互
  ...
  function onDocumentTouchMove(event) {
    // 单指拖拽
    if (_isUserInteracting === true && event.touches.length === 1) {
      _lon = (_onPointerDownPointerX - event.touches[0].clientX) * 0.1 + _onPointerDownLon;
      _lat = (event.touches[0].clientY - _onPointerDownPointerY) * 0.1 + _onPointerDownLat;
    }
    // 双指缩放
    else if (event.touches.length === 2) {
      var dx = event.touches[0].clientX - event.touches[1].clientX;
      var dy = event.touches[0].clientY - event.touches[1].clientY;
      var newPinchDistance = Math.sqrt(dx * dx + dy * dy);

      var m = _camera.getFocalLength();

      // 正确的缩放逻辑
      var scale = newPinchDistance / _initialPinchDistance;
      m *= scale;

      // 限制焦距
      var minFocalLength = options.minFocalLength;
      var maxFocalLength = options.maxFocalLength;
      m = Math.max(minFocalLength, Math.min(maxFocalLength, m));

      _camera.setFocalLength(m);

      _initialPinchDistance = newPinchDistance;
    }
  }

  function onDocumentTouchEnd(event) {
    _isUserInteracting = false;
    _initialPinchDistance = 0; // 重置距离
  }
  ...
  function onDocumentMouseWheel(ev, minFocalLength, maxFocalLength) {
    ev.preventDefault(); // 先阻止默认行为

    var m = _camera.getFocalLength();
    
    // ev.deltaY > 0 表示向下滚动（缩小）
    // ev.deltaY < 0 表示向上滚动（放大）
    // 焦距越大，视野越窄，等于放大
